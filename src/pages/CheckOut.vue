<template>
	<div style="padding-top: 72px;height:100vh">
		<div class="maxWidthPage grid grid-cols-5 h-full">
			...
			<div class="col-span-3 h-full py-20 px-16 flex flex-col">
				...
				<div class="flex flex-row gap-x-4 mb-4">
					<button class="px-4 py-2 bg-red-400 rounded-lg flex flex-row gap-x-2 focus:ring noOutline" @click="paymentChange('creditCard')">
						<svg
							class="w-6 h-6"
							fill="none"
							stroke="currentColor"
					...
					<button class="px-4 py-2 bg-green-400 rounded-lg flex flex-row gap-x-2 focus:ring noOutline" @click="paymentChange('wechat')">
						<svg
							class="w-6 h-6"
							fill="white"
							xmlns="http://www.w3.org/2000/svg"
					...
					<button class="px-4 py-2 bg-blue-400 rounded-lg flex flex-row gap-x-2 focus:ring noOutline" @click="paymentChange('alipay')">
						<img
							class="w-6 h-6"
				...
				<transition name="fade" mode="out-in" v-on:after-enter="injectElements">
					...
					<!-- WECHAT -->
					<div v-else-if="paymentMethod == 'wechat'" class="flex justify-center items-center" key="wechat">
...
<script lang="ts">
	...
	import {
		...
		StripeElementStyle, Stripe, StripeElements, SourceResult,
	...
	export default defineComponent({
		...
		async mounted() {
			//LOAD STRIPE
			const stripeTEMP = await loadStripe(
				"pk_test_51IXE3zJY5AEAzijm3OGvmcJ3bbhltgDPtIeyncJOS5QpDoLgje1k5Yhlu4mgkBTq9frLAhheD53R0M5CdlITtb9L00rDEjM4XV"
			);
			if (!stripeTEMP) {
		...
		methods: {
			// Payment Intents
			async createPaymentIntent(type: string) {
				//AMOUNT CHECK
			...
			async payNowWithCard(e: Event) {
				...
				let paymentIntent:PaymentIntent = await this.createPaymentIntent("card").catch(()=>{this.processing = false;return;});
			...
			//ELEMENT INJECTORS
			paymentChange(newPaymentMethod: string) {
			...
			injectElements() {
				switch (this.paymentMethod) {
					...
					default:
						console.error(
							"Inject Elements => New Payment Method does not Exist"
						);
				}
			},
			async weChatMount() {
				...
				if (!this.thingy.source?.wechat?.qr_code_url) {
					console.error("stripe wechat source problem");
					return;
				}
				...
				QRCode.toCanvas(
					canvas,
					this.thingy.source.wechat.qr_code_url,
					(error) => {
						if (error) {
							console.error(error);
							return;
			...
			async fetchFromAPI(endPointUrl: string, opts: Record<string, unknown>) {
				...
				const API =
					"http://localhost:5001/chicco-optical/us-central1/payments";
