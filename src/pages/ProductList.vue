<template>
	<div class="bg-yellow-400" v-if="gender">
		...
		<transition name="fade" mode="out-in">
			...
			<div key="loaded" class="px-10 py-12 grid grid-cols-3 gap-10" v-else>
				<item-card
	...
	export default defineComponent({
		...
		data:() => {
			return {
				...
				productType: window.location.pathname.slice(1).split("/")[0],
				gender: window.location.pathname.slice(1).split("/")[1],
				loaded:false,
			};
		},
		created(){
